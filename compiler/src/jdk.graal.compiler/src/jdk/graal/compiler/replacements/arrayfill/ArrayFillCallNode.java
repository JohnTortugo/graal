...
/**
 * Implements {@link Arrays#fill} via a {@linkplain ForeignCallNode call} to specialized stubs based
 * on the element type and memory properties.
 *
 * The target of the call is queried via {@link ArrayFillLookup#lookupArrayfillDescriptor}.
 * <p>
...
public final class ArrayFillCallNode extends AbstractMemoryCheckpoint implements Lowerable, SingleMemoryKill, MemoryAccess, Canonicalizable {

    public static final NodeClass<ArrayFillCallNode> TYPE = NodeClass.create(ArrayFillCallNode.class);
    ...
    private final ArrayFillForeignCalls foreignCalls;
    private final JavaKind wordJavaKind;
    private final int heapWordSize;

    /**
     * Aligned means that the offset of the fill is heap word aligned.
     */
    private boolean aligned;
    private boolean disjoint;
    private boolean uninitialized;

    public ArrayFillCallNode(@InjectedNodeParameter ArrayFillForeignCalls foreignCalls, @InjectedNodeParameter WordTypes wordTypes,
    ...
    public ArrayFillCallNode(@InjectedNodeParameter ArrayFillForeignCalls foreignCalls, @InjectedNodeParameter WordTypes wordTypes,
                    ValueNode src, ValueNode srcPos, ValueNode dest, ValueNode destPos,
                    ValueNode length, JavaKind fillKind, JavaKind srcKind, JavaKind destKind, boolean aligned, boolean disjoint, boolean uninitialized, int heapWordSize) {
        this(foreignCalls, wordTypes, src, srcPos, dest, destPos, length, fillKind, NamedLocationIdentity.getArrayLocation(srcKind), NamedLocationIdentity.getArrayLocation(destKind), aligned,
    ...
    public void lower(LoweringTool tool) {
        if (graph().getGuardsStage().areFrameStatesAtDeopts()) {
            ...
            ForeignCallDescriptor desc = foreignCalls.lookupArrayfillDescriptor(elementKind, isAligned(), isDisjoint(), isUninitialized(), killedLocationIdentity);
            assert desc != null : "no descriptor for arrayfill " + elementKind + ", aligned " + isAligned() + ", disjoint " + isDisjoint() + ", uninit " + isUninitialized() + ", killing " +
            ...
            assert callKills.length == 1 && callKills[0].equals(getKilledLocationIdentity()) : String.format("%s: fill of %s from %s should kill %s, unexpected kills: %s", call, elementKind,
    ...
    private static native void arrayfill(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter JavaKind elementKind, @ConstantNodeParameter boolean aligned,
    ...
    @NodeIntrinsic(hasSideEffect = true)
    private static native void arrayfill(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter JavaKind fillKind, @ConstantNodeParameter JavaKind srcKind,
    ...
    @NodeIntrinsic(hasSideEffect = true)
    private static native void arrayfill(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter JavaKind elementKind,
                    @ConstantNodeParameter LocationIdentity killedLocationIdentity, @ConstantNodeParameter boolean aligned,
                    @ConstantNodeParameter boolean disjoint, @ConstantNodeParameter boolean uninitialized, @ConstantNodeParameter int heapWordSize);

    public static void arrayfillObjectKillsAny(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter int heapWordSize) {
        arrayfill(src, srcPos, dest, destPos, length, JavaKind.Object, LocationIdentity.any(), false, false, false, heapWordSize);
    }

    public static void arrayfill(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter JavaKind elementKind,
                    @ConstantNodeParameter LocationIdentity killedLocationIdentity,
                    @ConstantNodeParameter int heapWordSize) {
        arrayfill(src, srcPos, dest, destPos, length, elementKind, killedLocationIdentity, false, false, false, heapWordSize);
    }

    public static void disjointArrayfill(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter JavaKind elementKind, @ConstantNodeParameter int heapWordSize) {
        arrayfill(src, srcPos, dest, destPos, length, elementKind, false, true, false, heapWordSize);
    }

    public static void disjointArrayfill(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter JavaKind elementKind,
                    @ConstantNodeParameter LocationIdentity killedLocationIdentity, @ConstantNodeParameter int heapWordSize) {
        arrayfill(src, srcPos, dest, destPos, length, elementKind, killedLocationIdentity, false, true, false, heapWordSize);
    }

    /**
     * Type punned fill of {@code length} elements of kind {@code fillKind} from an array with
     * {@code srcKind} elements to an array with {@code destKind} elements.
     */
    public static void disjointArrayfillDifferentKinds(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter JavaKind fillKind, @ConstantNodeParameter JavaKind srcKind,
                    @ConstantNodeParameter JavaKind destKind, @ConstantNodeParameter int heapWordSize) {
        arrayfill(src, srcPos, dest, destPos, length, fillKind, srcKind, destKind, false, true, false, heapWordSize);
    }

    public static void disjointArrayfillKillsInit(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter JavaKind elementKind, @ConstantNodeParameter int heapWordSize) {
        arrayfill(src, srcPos, dest, destPos, length, elementKind, LocationIdentity.init(), false, true, false, heapWordSize);
    }

    public static void disjointUninitializedArrayfill(Object src, int srcPos, Object dest, int destPos, int length, @ConstantNodeParameter JavaKind elementKind,
                    @ConstantNodeParameter int heapWordSize) {
        arrayfill(src, srcPos, dest, destPos, length, elementKind, false, true, true, heapWordSize);
    }
    ...
    public void updateAlignedDisjoint(MetaAccessProvider metaAccess) {
        ...
        if (constantSrc != null && constantDst != null) {
            ...
            if (constantSrc.asInt() >= constantDst.asInt()) {
                // low to high fill (filling forwards) so treat as disjoint
                disjoint = true;
            }
        }
    }
