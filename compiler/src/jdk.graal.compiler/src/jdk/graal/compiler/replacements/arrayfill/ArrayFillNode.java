...
public abstract class ArrayFillNode extends WithExceptionNode
    ...
    public ArrayFillNode(NodeClass<? extends ArrayFillNode> type, ValueNode src, ValueNode srcPos, ValueNode dest, ValueNode destPos, ValueNode length, JavaKind elementKind, int bci) {
    ...
    public ArrayFillNode(NodeClass<? extends ArrayFillNode> type, ValueNode src, ValueNode srcPos, ValueNode dest, ValueNode destPos, ValueNode length, JavaKind elementKind) {
        this(type, src, srcPos, dest, destPos, length, elementKind, BytecodeFrame.INVALID_FRAMESTATE_BCI);
    }

    protected ArrayFillNode(int bci, ValueNode src, ValueNode srcPos, ValueNode dst, ValueNode dstPos, ValueNode length, boolean forceAnyLocation) {
        this(TYPE, src, srcPos, dst, dstPos, length, null, bci);
    ...
    public static JavaKind selectComponentKind(ArrayFillNode arrayfill) {
        if (arrayfill.getSource() == null || arrayfill.getDestination() == null) {
            return null;
        }
        ResolvedJavaType srcType = StampTool.typeOrNull(arrayfill.getSource().stamp(NodeView.DEFAULT));
        ResolvedJavaType destType = StampTool.typeOrNull(arrayfill.getDestination().stamp(NodeView.DEFAULT));
    ...
    /*
     * Returns true if this fill doesn't require store checks. Trivially true for primitive arrays.
     */
     ...
     * {@code Illegal} values. That also means such a write must be treated as a group by arrayfill
     * so reads and writes can't be performed in the middle of the illegal values.
     * <p>
     * To ensure that an arrayfill properly treats these kinds of writes as group, it's sufficient
            ...
            if (destAlias instanceof VirtualArrayNode) {
                ...
                if (srcAlias instanceof VirtualObjectNode) {
                    ...
                    if (srcVirtual == destVirtual && srcPosInt < destPosInt) {
                        // must fill backwards to avoid losing elements
                    ...
                    if (debug.isLogEnabled()) {
                        debug.log("virtualized arrayfill(%s, %d, %s, %d, %d)", getSource(), srcPosInt, getDestination(), destPosInt, len);
                    }
                } else {
